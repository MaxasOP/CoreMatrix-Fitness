...
function LayoutContent({ children }) {
  ...
  const visibleNavItems = nav.slice(0, 4);
  const moreNavItems = nav.slice(4);
  ...
  return (
    <div className="min-h-screen text-gray-900 font-sans selection:bg-[#ff5a1f]/20 relative z-0">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1 bg-gray-50/80 p-1.5 rounded-2xl border border-gray-100">
            {nav.map(n => {
              ...
              return (
                <Link key={n.to} href={n.to} className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 ${isActive ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100/50'}`}>
                  {n.label}
                </Link>
              );
            })}
          ...
          <div className="flex items-center gap-4">
            {user ? (
              ...
              <Link href="/auth" className="bg-[#ff5a1f] hover:bg-orange-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-md shadow-orange-500/20 hover:-translate-y-0.5 hover:shadow-orange-500/30">Sign in</Link>
            )}
          </div>
        </div>
      </header>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden pb-safe">
        <div className="bg-white/90 backdrop-blur-xl border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] px-4 py-3 flex items-center justify-around">
          {visibleNavItems.map(n => {
            const isActive = pathname === n.to;
            return (
              <Link key={n.to} href={n.to} className={`flex flex-col items-center gap-1.5 transition-colors ${isActive ? 'text-[#ff5a1f]' : 'text-gray-400 hover:text-gray-600'}`} onClick={() => setShowMoreNav(false)}>
      ...
      {showMoreNav && (
        <div className="fixed inset-0 z-50 md:hidden bg-black/50 backdrop-blur-sm flex items-end justify-center" onClick={() => setShowMoreNav(false)}>
          <div className="bg-white rounded-t-3xl shadow-[0_-8px_30px_rgba(0,0,0,0.1)] w-full max-h-[80vh] overflow-y-auto pb-safe p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-3 gap-4">
              {moreNavItems.map(n => {
                const isActive = pathname === n.to;
                return (
                  <Link key={n.to} href={n.to} className={`flex flex-col items-center gap-1.5 p-3 rounded-xl transition-colors ${isActive ? 'bg-orange-50 text-[#ff5a1f]' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`} onClick={() => setShowMoreNav(false)}>
...
export default function ClientLayout({ children }) {
  // Safe client-side dark theme initialization
  useEffect(() => {
    try {
      var theme = localStorage.getItem('cmTheme');
